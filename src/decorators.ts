/**
 * These decorators all apply the information they collect (whether class,
 * method, or parameter data) as tagged metadata on the class's constructor
 */
...
import {
  ...
  CONTAINER_ID,
  INJECT_METADATA_PROP_KEY
} from "./constants"
import { isClass, isNumber } from "@3fv/guard"
...
/**
 * Placed just before the class declaration, this class decorator applies
 * metadata to the class constructor indicating that the user intends to bind
 * the class into the container. This decorator will throw if not placed on a
 * class declaration, or if placed more than once on a class declaration.
 */
export function Injectable<T, Options extends ProviderOptions<T> = {}>(options: Options = {} as Options) {
  /**
   * @param target   The constructor function of the class that is being
   *   decorated
   * @returns Undefined (nothing), as this decorator does not modify the
   *   constructor in any way.
   */
...
/**
 * Placed just before a constructor parameter, this parameter decorator allows
 * for specificity and control over the type of the type of Object that will be
 * injected into the parameter. In the absence of this decorator the container
 * will use whatever is bound to a parameter's type (or throw an error if it is
...
export function Inject(id: InjectableId<any>) {
  /**
   * @param target  The constructor function of the class (we don't allow
   *   @Inject on anything else).
   * @param parameterName The name of the parameter
   * @param parameterIndex The ordinal index of the parameter in the function’s
   *   parameter list
   * @returns Undefined (nothing), as this decorator does not modify the
...
/**
 * This is a helper function used by the container to retrieve the @Inject
 * metadata for a specifically indexed constructor parameter
 * @param target  The constructor function of the class (we don't allow @Inject
 *   on anything else).
 * @param parameterIndex    The ordinal index of the parameter in the
...
/**
 * Placed just before a constructor parameter, this parameter decorator signals
 * the container that it should supply the 'alt' constant value (undefined by
 * default) if for *any* reason it is unable to otherwise resolve the type of
 * the parameter. WARNING!  It is your responsibility to ensure that alt is of
...
/**
 * This is a helper function used by the container to retrieve the @Optional
 * metadata for a specifically indexed constructor parameter
 * @param target  The constructor function of the class (we don't allow
 *   @Optional on anything else).
 * @param parameterIndex    The ordinal index of the parameter in the
 *   constructor’s parameter list
 * @see Optional
 * @returns an object containing the value provided in the decorator, or
 *   undefined if no annotation was present.
 */
...
/**
 * Placed just before a class method, this method decorator flags a method that
 * should be called after an object has been instantiated by the container, but
 * before it is put into service. The method will be assumed to be synchronous
 * unless the method signature explicitly declares it's return type to be ":
 * Promise<something>" This decorator will throw if placed on a non-method or a
 * static method of a class, or if placed on a method more than once, or if
 * placed on more than one method for a class.
 */
